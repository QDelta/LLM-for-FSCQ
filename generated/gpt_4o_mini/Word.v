...
apply Nat.lt_trans with (wordToNat w * 2).
apply Nat.lt_trans with (wordToNat (WS b w)).
apply Nat.lt_trans with (wordToNat (WS b w)).
apply Nat.lt_trans with (wordToNat (WS b w)).
apply Nat.lt_trans with (wordToNat (WS b w)).
apply Nat.lt_trans with (wordToNat (WS b w)).
apply Nat.lt_trans with (wordToNat (WS b w) * 2).
...
simpl. induction w1; simpl; intros.  
- reflexivity.
...
refine (fun sz w1 w2 f n => _).
unfold wlshift, split1, bitwp, combine. reflexivity.
Qed.
