...
intros Fd f pfy ufy fy i j data Hvalid_prot Hvalid_unified Hvalid_byte Harray Hj Hlen_data Hlen_bound Hattr Hbytes Hroundup.
(* No more tactics to try *)
Admitted.
...
intros f pfy ufy fy block_off m1 nb data Fd Hpfy Hufy Hfy Hpred Hnb Hlength Hm1.
(* No more tactics to try *)
Admitted.
...
repeat intros; repeat (rewrite ?length_map, ?firstn_length, ?skipn_length; try lia).
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 65 *)
simpl. unfold merge_bs. destruct i; auto.
unfold merge_bs. destruct l; simpl; intros; auto.
(* No more tactics to try *)
Admitted.
...
intros f pfy ufy fy block_off byte_off data F H_valid_proto H_valid_unified H_valid_byte H_attr_length H_roundup H_mem H_byte_off_lt H_data_length.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 42 *)
intros f pfy i Hvalid Hlen.
unfold proto_bytefile_valid in Hvalid.
unfold DFData in Hlen.
unfold PByFData in Hlen.
rewrite Hvalid.
auto.
rewrite <- Hvalid.
(* No more tactics to try *)
