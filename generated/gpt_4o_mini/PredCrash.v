...
unfold crash_xform; repeat (split; intros; simpl).
unfold_sep_star.
eapply sep_star_lift_l.
repeat (apply pimpl_exists_l_star; intros).
apply sep_star_comm.
eapply sep_star_lift_l.
intros.
...
apply sep_star_lift_l.
apply pimpl_exists_l_star; intros.
apply sep_star_comm.
apply sep_star_comm.
...
unfold crash_xform; intros; firstorder.
Qed.
...
unfold crash_xform.
intros; eauto.
intros. unfold crash_xform. 
unfold possible_crash in *. 
eauto.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 9 *)
refine (let p := _ in let q := _ in _).
refine (let p := crash_xform p in let q := crash_xform q in _).
refine (let q := crash_xform q0 in let p := crash_xform p0 in _).
refine (let p := crash_xform p0 in let q := crash_xform q0 in _).
refine (let p := crash_xform p0 in let q := crash_xform q0 in _).
refine (let p := _ in let q := _ in _).
refine (let p := _ in let q := _ in _).
refine (let p := _ in let q := _ in _).
...
do 2 unfold crash_xform.
unfold sep_star; unfold diskIs; simpl; intros; pred.
unfold_sep_star.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 28 *)
intros AT AEQ m. unfold possible_sync. intros a. left. split; auto.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 39 *)
repeat unfold sync_xform, sync_invariant; intros.
...
Proof.
(* original proof tokens: 9 *)
unfold sync_xform; simpl; repeat (apply pimpl_or; auto).
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 34 *)
unfold sync_xform; simpl; repeat (rewrite <- !sep_star_exists_l); eauto.
(* No more tactics to try *)
Admitted.
...
(* original proof tokens: 9 *)
repeat intro.
eapply sync_xform_pimpl; [apply H |].
(* No more tactics to try *)
Admitted.
