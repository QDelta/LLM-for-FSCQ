...
intros A AEQ m m' a v l l' Hsync Hincl.
apply possible_sync_trans with (m2 := m').
apply possible_sync_trans with (m2 := m').
apply possible_sync_trans with (m2 := upd m a (v, l)).
apply possible_sync_trans with (m2 := upd m a (v, l)).
apply possible_sync_trans with (m2 := upd m' a (v, l')).
apply possible_sync_trans with (m2 := m').
apply possible_sync_trans with (m2 := m').
...
split; intros; destruct out; destruct out'; simpl in *; intuition; subst; repeat match goal with
| [ H : exists d', _ |- _ ] => destruct H as [d' H]; destruct H; subst
| [ H : exists d, _ |- _ ] => destruct H as [d H]; destruct H; subst
| [ H : _ /\ _ |- _ ] => destruct H
end; repeat (exists d; intuition).
exists m.
(* First apply auto to retrieve the necessary contradictions to arrive at the goals required. *)
auto.
exists m.
split.
admit.
...
refine (Build_PreOrder _ _ _).
- (* Reflexivity *)
  intros x. apply outcome_obs_le_refl.
- (* Transitivity *)
  intros x y z Hxy Hyz. apply outcome_obs_le_trans with y; assumption.
Defined.
...
(* original proof tokens: 15 *)
split.
induction out as [| |]; intros; simpl; try constructor; auto.
...
induction out; intros; subst; simpl in *; try (exists (RFailed TR); split; auto);
  try (exists (RRecovered TR d0 vm hm v); split; auto);
  try (exists (RFinished TR d0 vm hm v); split; auto);
  try (exists (RFinished TR d0 vm hm v); split; auto).
exists (RFailed TF TR); split; [ constructor | constructor; auto ].
destruct H.
induction H; intros; subst; try (exists (RFailed TR TR); split; auto);
  try (exists (RRecovered TR m vm0 hm0 v); split; auto);
  try (exists (RFinished TR m vm0 hm0 v); split; auto).
constructor.
(* No more tactics to try *)
Admitted.
...
intros TF TR p r d vm hm out H. 
induction H; eauto.
