...
intros Fm Ftop fsxp mscs Ftree srcpath tmppath srcinum file dstbase dstname dstfile ts ds sm Hds Hsrc.
(* No more tactics to try *)
Admitted.
...
intros.
repeat split; auto.
apply treeseq_pred_tree_rep_dir2flatmem2 in H.
(* No more tactics to try *)
Admitted.
...
intros F f fy off data Hrep Hoff Hpred Hdata.
contradiction Hoff.
(* No more tactics to try *)
Admitted.
...
(* original proof tokens: 438 *)
intros. 
repeat (length_rewrite_l; auto; 
        rewrite Nat.mul_comm; 
        apply Nat.mul_le_mono_r; try lia).
(* No more tactics to try *)
Admitted.
...
| [ H: context[dsupd ?ds ?bn ?v] |- _ ] => 
    unfold dsupd in H; simpl in H; destruct H
| [ H: context[dsupd_iter ?ds ?bn ?v] |- _ ] => 
    unfold dsupd_iter in H; simpl in H; destruct H
| [ H: context[combine ?l ?r] |- _ ] => 
    unfold combine in H
end.
(* No more tactics to try *)
Admitted.
...
unfold read_middle, read_first, read_middle_blocks, read_last.
repeat match goal with
| [ |- context[if ?x then _ else _] ] => destruct x; simpl
| [ |- context[if ?x then _ else _] ] => destruct x; simpl; try lia; auto
end.
intros; 
unfold read, read_first, read_middle, read_last, read_middle_blocks; 
repeat match goal with
| [ |- context[if ?x then _ else _] ] => destruct x; simpl
| [ |- context[if ?x then _ else _] ] => destruct x; simpl; try lia; auto
end.
...
unfold atomic_cp; unfold copy_and_rename; unfold copydata.
(* No more tactics to try *)
Admitted.
