...
intros t pn1 pn2 inum2 f2 Hsubtree Hnotfound Hprefix.
apply pathname_prefix_neq in Hprefix; auto.
(* No more tactics to try *)
Admitted.
...
intros t pn1 old pn2 inum1 f1 inum2 f2 Hfind Hold Hneq Hprefix.
contradict Hprefix.
(* No more tactics to try *)
Admitted.
...
repeat intros; unfold treeseq_in_ds in *; simpl in *; intuition.
(* No more tactics to try *)
Admitted.
...
intros F Ftop fsxp sm mscs ts ds n pathname bn off v inum f Hsubtree Hbelong Hds Hrep Hsafe.
(* No more tactics to try *)
Admitted.
...
intros F Ftop fsxp sm mscs ts ds pathname bn off v inum f 
       H_find H_block H_in_ds H_safe.
...
Proof.
(* original proof tokens: 900 *)
intros Fm fsxp Ftop mscs mscs' Ftree sm ts ds n pathname pathname' f f' off v inum bn Hrep Hfile Halloc Htrue Hbelong Hsafe Hsafe' Hdistinct Hdistinct' Htree.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 58 *)
intros off f off' v f' H.
unfold synced_file_alt_helper in *.
induction off; simpl; auto.
unfold synced_file_alt_helper in *.
simpl.
induction off; simpl; auto.
(* No more tactics to try *)
...
intros pathname f tree fsxp F F0 ilist freeblocks mscs sm inum m al tree_newest Hfind_length Hblock Hrep.
(* No more tactics to try *)
Admitted.
...
intros; 
    repeat match goal with 
      | [ H: context [ find_subtree ?x (TStree ?ts !!) = Some _ ] |- _ ] =>
        destruct (find_subtree_spec H)
      | [ H: context [ find_dirlist ?x ?y = Some _ ] |- _ ] =>
        destruct (find_dirlist_spec H)
      | [ H: context [ dirtree_inum ?x = ?y ] |- _ ] =>
        destruct (dirtree_inum_spec H)
      | [ H: context [ dirtree_isdir ?x = true ] |- _ ] =>
        destruct (dirtree_isdir_spec H)
      | [ H: context [ tree_names_distinct (TStree ?ts !!) ] |- _ ] =>
        apply tree_names_distinct in H
      | [ H: context [ tree_inodes_distinct (TStree ?ts !!) ] |- _ ] =>
        apply tree_inodes_distinct in H
      | [ H: context [ find_subtree _ (TreeDir _ _) = Some _ ] |- _ ] =>
        subst; simpl; auto
    end; clear.
split.
unfold treeseq_safe_fwd; intros.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 353 *)
unfold treeseq_one_file_sync_alt, treeseq_one_file_sync, treeseq_one_upd.
(* No more tactics to try *)
Admitted.
