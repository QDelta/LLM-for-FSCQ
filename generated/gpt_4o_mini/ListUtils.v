...
split; intros; destruct l; simpl in *; try lia.
firstorder.
destruct l.
intros; discriminate.
intros; discriminate.
...
Proof.
(* original proof tokens: 32 *)
induction al as [|a al' IH]; intros; simpl.
induction bl as [|y tl']; simpl.
induction br as [|b br' IH]; simpl; auto.
induction br.
rewrite <- IHtl'.
rewrite IHtl' in *.
rewrite <- IHtl'.
rewrite IHtl' in *; auto.
rewrite <- IHtl' in *.
rewrite IHtl'.
rewrite <- IHtl'.
rewrite IHtl' in *.
...
Proof.
(* original proof tokens: 48 *)
induction a as [| x xs IH]; intros b i Ha Hb Hlen.
simpl.
apply Nat.succ_inj.
apply Nat.succ_inj.
apply Nat.succ_inj.
...
Proof.
(* original proof tokens: 49 *)
destruct a; intros; simpl; firstorder.
rewrite <- H0.
destruct b as [|y0 tl']; [ destruct H; auto | ].
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 21 *)
simpl. induction l; intros; simpl.
(* No more tactics to try *)
Admitted.
...
intros A m n a. induction m; simpl.
induction n; simpl.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 32 *)
(* generated proof tokens: 126 *)

cut (forall (T : Type) (a : T) (l1 l2 : list T),
  In a (l1 ++ l2) -> NoDup (l1 ++ l2) -> ~ In a l2 -> In a l1).
- intros H; apply H; auto.
- intros T a l1 l2 HIn HNoDup HNotIn.
    apply in_app_or in HIn. destruct HIn as [HInL | HInR].
    + assumption.
    + exfalso. apply HNotIn. assumption. 
Qed.
...
induction i; simpl; try rewrite selN_cons; reflexivity.
Qed.
...
Proof.
(* original proof tokens: 47 *)
induction l as [|x l' IH]; simpl; split; intros; try constructor; auto.
induction H.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 34 *)
intros A B la lb P H HP. induction la as [| a la' IH].
induction HP.
...
intros A l1 B l2 T1 T2 p q f1 f2 Hp Hq.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 140 *)
intros A l1 B l2 T1 T2 p q f1 f2 def1 def2 Hforall Hq.
specialize (Hforall def1 def2).
(* No more tactics to try *)
Admitted.
...
intros T U l l' R f HHR H; induction H; subst; simpl; auto.
apply HHR in H.
rewrite H.
apply (f_equal (fun z => f y :: z)).
apply IHForall2.
Qed.
...
intro A; intro l; intro n; unfold cuttail. 
...
simpl.
induction H.
simpl; rewrite cuttail_0; reflexivity.
induction H.
simpl.
(* No more tactics to try *)
Admitted.
...
intros T a b v H. unfold incl. intros x HIn. destruct HIn as [HIn_h | HIn_t]. 
- left. apply HIn_h. 
...
intros T l start v. simpl. reflexivity.
Qed.
...
Proof.
(* original proof tokens: 40 *)
intros T l start len x v.
replace (S start) with (start + 1) in *; [| lia].
(* No more tactics to try *)
Admitted.
...
unfold postfix. exists 0. reflexivity.
Qed.
...
split; intros; [apply NoDup_rev_1 | apply NoDup_rev_2]; assumption.
Qed.
...
Proof.
(* original proof tokens: 44 *)
induction vs as [| x rest IH].
induction start as [| start' IH].
intros r d H.
firstorder.
apply False_ind; subst; inversion H.
induction H.
intuition.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 54 *)
intros T v off l def Hlen Hsame.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 63 *)
intros T l1 l2 l x H NoDupInclusion.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 216 *)
intros S T f t l x s0 t0 Hnodup_t Hnodup_l Hexists Hf Hin.
split.
(* No more tactics to try *)
Admitted.
