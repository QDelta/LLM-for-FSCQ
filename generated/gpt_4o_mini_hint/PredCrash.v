...
intros p m m' Hcrash Hp. unfold crash_xform. exists m'. split; auto.
...
unfold crash_xform, pimpl, foral_; intros.
repeat deex.
eexists; split; eauto.
Qed.
...
unfold crash_xform, diskIs; pred.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 61 *)
unfold sync_xform, piff; split; intros; repeat deex; repeat eexists; intuition eauto.
(* No more tactics to try *)
Admitted.
...
unfold mem_disjoint, possible_sync; intros.
intro Hnot.
repeat deex.
destruct (H0 a); destruct (H1 a); try intuition congruence.
repeat deex. apply H.
...
repeat unfold sync_invariant, emp, pimpl, lift_empty; intros; intuition.
(* No more tactics to try *)
Admitted.
...
specialize (H a).
specialize (H0 a).
destruct (m a); intuition; repeat deex; try congruence.
right; repeat deex; repeat eexists; intuition eauto.
assert (In v' (vsmerge p)).
(* No more tactics to try *)
Admitted.
