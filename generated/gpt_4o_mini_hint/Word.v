...
apply Nat.induction; intros; simpl in *; auto.
apply nat_ind; intros; simpl in *; auto.
(* No more tactics to try *)
Admitted.
...
refine (fun a b c d w H => _).
refine (fun H => _).
refine (let H1 := eq_rect_combine_dist_helper1 a b c H0 in _).
refine (let H1 := eq_rect_combine_dist_helper1 a b c H0 in _).
refine (let H1 := eq_rect_combine_dist_helper1 a b c H0 in
        let H2 := eq_rect_combine_dist_helper2 a b c H0 in
        let w' := eq_rec ((a + b) * c) word w _ H1 in
        _).
replace w' with (eq_rec ((a + b) * c) word w (a * c + d) (eq_rect_combine_dist_helper1 a b c H0)).
simpl.
(* No more tactics to try *)
Admitted.
...
induction n1; intros; simpl; auto.
admit.
(* Reached max number of model calls *)
Admitted.
...
Proof.
(* original proof tokens: 25 *)
unfold wor, wones; induction w; simpl; intuition.
(* No more tactics to try *)
Admitted.
...
unfold wxor, wnot, bitwp, wnot', wones. 
