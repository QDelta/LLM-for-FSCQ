...
intros.
eapply treeseq_pred_impl with (p := tree_rep Ftree srcpath tmppath srcinum file tinum dstbase dstname dstfile) in H.
eapply treeseq_pred_impl; eauto.
eapply treeseq_pred_impl with (p := tree_rep Ftree srcpath tmppath srcinum file tinum dstbase dstname dstfile) in H.
eapply treeseq_pred_impl with (p := tree_rep Ftree srcpath tmppath srcinum file tinum dstbase dstname dstfile) in H.
eapply treeseq_pred_impl; try eassumption.
(* No more tactics to try *)
Admitted.
...
case_eq (find_subtree tmppath (TStree t)); intros; subst; auto.
destruct d; auto.
(* No more tactics to try *)
Admitted.
...
induction 1.
unfold copydata, AFS.file_get_attr, AFS.read_fblock, AFS.update_fblock_d, AFS.file_set_attr in *.
(* No more tactics to try *)
Admitted.
