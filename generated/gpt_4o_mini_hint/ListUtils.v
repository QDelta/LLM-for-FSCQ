...
induction l; destruct n; simpl; intros; firstorder.
Qed.
...
induction l; intros; simpl in *; destruct idx; try lia; inversion H.
...
unfold firstn. induction n; intros; simpl. auto. 
...
apply selN_selN_def_eq; assumption.
Qed.
...
Proof.
(* original proof tokens: 25 *)
split; intros; destruct l; simpl; try lia. contradiction. auto.
(* No more tactics to try *)
Admitted.
...
split; intros; destruct l; simpl in *; intuition.
intros; discriminate.
inversion H0.
Qed.
...
(* original proof tokens: 29 *)
firstorder.
simpl in H0; destruct l; auto.
simpl in H0; destruct l; auto.
apply length_not_nil' in H0.
firstorder.
apply length_not_nil in H0.
firstorder.
apply length_not_nil in H0.
...
intros A n len1 len2 l H.
induction len1; intros; simpl.
destruct (skipn n (firstn len2 l)) eqn: Hskip1; destruct (skipn n l) eqn: Hskip2; simpl; auto.
(* No more tactics to try *)
Admitted.
...
intros A B a b i Hia Hib Hlen.
rewrite removeN_length in Hlen; [ | lia ].
rewrite removeN_length in Hlen; [ | lia ].
lia.
Qed.
...
Proof.
(* original proof tokens: 40 *)
induction l; split; intros; constructor; auto.
(* No more tactics to try *)
Admitted.
...
intros A l1 B l2 T1 T2 p q f1 f2 H1 H2.
...
intros A x l1 l2 H H0. apply H0 in H. auto.
Qed.
...
Proof.
(* original proof tokens: 47 *)
apply well_founded_ind with (R := fun x y => x >= y).
apply well_founded_ind with (R := fun x y => x >= y).
apply (well_founded_ind (R := fun x y => x >= y)).
apply well_founded_ind with (R := fun x y => x >= y).
apply well_founded_ind with (R := fun x y => x >= y).
apply well_founded_ind with (R := fun x y => x >= y).
apply well_founded_ind with (R := fun x y => x >= y).
...
inversion H; subst; apply ListSameCons; auto.
Qed.
...
split.
- apply H; left; reflexivity.
- intros x H1. apply H; right; assumption.
Qed.
...
intros T E l. unfold incl_count. reflexivity.
Qed.
...
Proof.
(* original proof tokens: 9 *)
unfold NoDupApp; split; intros; simpl in *; inversion H; auto.
(* No more tactics to try *)
Admitted.
