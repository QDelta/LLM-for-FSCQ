...
intros. unfold diskIs. reflexivity.
Qed.
...
intros A l def H.
destruct l as [| x xs]; try contradiction.
reflexivity.
Qed.
...
intros.
unfold merge_bs.
destruct l, l'; simpl; try lia.
inversion H.
destruct H.
unfold byteset0, list2byteset.
(* No more tactics to try *)
Admitted.
...
intros f fy fy' n Hlen Hbytes Hattr Hgood Hmod.
apply Hlen. auto.
cut (length (ByFData fy) > 0).
(* No more tactics to try *)
Admitted.
