...
apply functional_extensionality.
intros x.
destruct (AEQ x a) as [H1 | H1].
rewrite updSome_ne in *; auto.
(* No more tactics to try *)
Admitted.
...
apply functional_extensionality.
intros x.
destruct (AEQ x a0); destruct (AEQ x a1).
...
intros x.
destruct (AEQ x a0); destruct (AEQ x a1).
rewrite updSome_ne in |- * by congruence.
rewrite updSome_ne in |- * by congruence.
rewrite updSome_ne in |- * by congruence.
rewrite updSome_ne.
reflexivity.
...
apply functional_extensionality.
intros x.
destruct (AEQ x a0) as [H1|H1].
destruct (AEQ x a1) as [H2|H2].
rewrite H1.
(* No more tactics to try *)
Admitted.
