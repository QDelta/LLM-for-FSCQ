...
induction a; intros; induction b; intros.
...
exists (0); auto.
destruct (Nat.div2 w) eqn:E.
destruct (mod2 w); simpl.
rewrite div2_even in E; try lia.
rewrite div2_odd in E; try lia.
(* No more tactics to try *)
Admitted.
...
induction sz; intros; auto.
exists 0.
rewrite roundTrip_0.
destruct (mod2 w) eqn:Hmod2.
split.
(* No more tactics to try *)
Admitted.
...
induction w1; simpl; auto; intros; simpl in *; f_equal; auto.
(* No more tactics to try *)
Admitted.
...
induction sz1; simpl; auto; intros; rewrite IHsz1; simpl; auto.
Qed.
...
destruct sz; simpl; auto; try congruence.
rewrite IHp; simpl.
(* No more tactics to try *)
Admitted.
...
induction sz; simpl; auto; congruence.
...
simpl.
auto.
destruct (le_lt_dec (2 * k) (S n)); try lia.
...
Proof.
(* original proof tokens: 25 *)
destruct n; destruct k; simpl; auto; try congruence.
intros.
inversion H.
inversion H.
inversion H.
...
inversion H.
inversion H.
destruct k; simpl; auto; try congruence.
inversion H.
destruct k; simpl; auto; try congruence.
(* No more tactics to try *)
Admitted.
...
unfold wminus, wminusN, wplus, wplusN, wneg, wnegN.
reflexivity.
Qed.
...
Proof.
(* original proof tokens: 14 *)
induction x; intros; simpl in *; try (auto); try (inversion H).
destruct (eqb_spec b (whd y)); simpl in *; try (auto); try (inversion H).
...
Proof.
(* original proof tokens: 19 *)
induction x; simpl; auto; try lia.
(* No more tactics to try *)
Admitted.
...
induction a; intros; simpl in *; try congruence.
inversion H.
inversion H.
inversion H.
inversion H.
...
intros.
apply lt_word_lt_nat in H; auto.
Qed.
...
left; apply eq_refl.
right.
exists (wordToNat n0 * 2 + (if b then 1 else 0)).
split.
(* No more tactics to try *)
Admitted.
