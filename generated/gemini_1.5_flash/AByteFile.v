...
intros.
extensionality a'.
destruct (le_dec a a'); simpl; auto.
destruct (lt_dec a' (a + S n)); simpl; auto.
rewrite mem_except_ne; auto.
unfold mem_except.
simpl.
destruct (addr_eq_dec a' a); auto.
destruct (le_dec (S a) a') ; simpl.
destruct (lt_dec a' (S (a + n))); simpl; auto.
destruct (le_dec (S a) a') ; simpl.
destruct (le_dec (S a) a'); simpl; try lia.
destruct (le_dec (S a) a'); simpl.
destruct (le_dec (S a) a') ; simpl.
inversion l0; subst; try lia.
destruct (le_dec (S a) a'); simpl.
inversion l1; subst; try lia.
destruct (le_dec (S a) a'); simpl.
destruct (le_dec (S a) a'); simpl.
...
unfold upd_range, list2nmem; simpl.
(* No more tactics to try *)
Admitted.
...
intros.
unfold upd_range in H0.
rewrite upd_range_list2nmem_comm in H0; [|simplen].
...
unfold valu0, valu2list, bytes2valu, valu2bytes.
rewrite valulen_is.
(* No more tactics to try *)
Admitted.
