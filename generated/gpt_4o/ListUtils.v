...
intros A l P def H; induction l as [| x xs IH].
- constructor.
-
constructor.
specialize (H 0).
...
intros A B la lb P Hlen Hforall.
induction Hforall as [| x lb' HPx IH].
(* No more tactics to try *)
Admitted.
...
intros A B la lb P Hforall Hlength.
(* No more tactics to try *)
Admitted.
...
intros A B a b P n ad bd HForall2 Hlen.
unfold selN.
revert Hlen.
induction HForall2; simpl; intros.
destruct n.
lia.
destruct n.
(* Reached max number of model calls *)
...
intros A B a b P da db Hlen Hforall.
revert b Hlen Hforall.
intros b Hlen Hforall.
revert a b Hlen Hforall.
induction a as [|ha ta IHa]; intros [|hb tb] Hlen Hforall.
- constructor.
-
discriminate Hlen.
-
...
-
constructor.
apply (Hforall 0); simpl; lia.
apply IHa.
inversion Hlen; subst; auto.
intros i Hi.
apply (Hforall (S i)); simpl; lia.
Qed.
...
simpl; rewrite cuttail_0.
apply Forall_inv in Hl.
simpl; rewrite cuttail_0 in IHl'; auto.
(* Reached max number of model calls *)
Admitted.
...
intros T l1 l2 a Hnin Hincl x Hx.
specialize (Hincl x (or_intror Hx)).
inversion Hincl; subst; [contradiction Hnin | assumption].
Qed.
...
intros T l1 l2 a H_incl H_not_in.
unfold incl in *.
intros a0 H_in_l1.
specialize (H_incl a0 H_in_l1).
apply in_inv in H_incl as [H_eq | H_in]; [exfalso; subst; apply H_not_in; assumption | assumption].
Qed.
...
(* generated proof tokens: 49 *)
intros T E l1 l1' l2 l2' H1 H2 x.
rewrite count_occ_app.
rewrite count_occ_app.
apply Nat.add_le_mono; apply H1 || apply H2.
Qed.
