...
intros v. unfold word2val, val2word. rewrite <- blocksz_ok. reflexivity.
Qed.
...
rewrite <- firstn_skipn with (n := n) (l := firstn (n + i * n) l) at 1.
rewrite <- firstn_skipn with (n := n) (l := firstn (n + i * n) l) at 1.
rewrite <- firstn_skipn with (l := firstn (n + i * n) l) (n := n) at 2.
simpl.
f_equal; auto.
rewrite <- firstn_skipn with (n := n) (l := l) at 1.
rewrite <- firstn_skipn with (n := n) (l := firstn (n + i * n) l).
rewrite <- firstn_skipn with (n := n) (l := l) at 1.
