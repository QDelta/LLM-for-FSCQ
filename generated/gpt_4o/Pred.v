...
intros m1 m2 a v Hdisjoint Ha.
unfold mem_disjoint in Hdisjoint.
contradict Hdisjoint; exists a, v, v.
split; [assumption |].
...
intros a b.
intros m [Ha Hb].
apply sep_star_lift_apply'; assumption.
Qed.
...
intros a b c H.
intros m Ha; destruct H as [H | H]; [left; apply H | right; apply H]; assumption.
Qed.
...
intros F a a' v m Hsep Hneq.
apply sep_star_ptsto_indomain in Hsep.
apply ptsto_mem_except_F with (F := F) (a' := a'); eauto.
apply mem_except_ptsto.
destruct Hsep as [v' Hsome]; rewrite Hsome.
...
intros m a a'.
unfold mem_except.
apply functional_extensionality; intros a'0.
destruct (AEQ a'0 a);
destruct (AEQ a'0 a');
simpl; auto.
Qed.
...
intros a v H.
unfold emp, pimpl in H.
specialize (H (empty_mem) emp_empty_mem).
apply ptsto_empty_mem in H; contradiction.
Qed.
...
intros Himpl.
intros m Hm; apply Hx0y0; apply Himpl; apply Hxy; assumption.
Qed.
...
intros p1 p2 Hpiff q1 q2 Hqiff m1 m2 Heq Hstar.
subst.
rewrite <- Hpiff.
rewrite <- Hqiff.
assumption.
...
intros P P' HP Q Q' HQ. subst. unfold Basics.impl, lift_empty in *. intros [HP' HQ']. split; auto.
Qed.
...
(* generated proof tokens: 50 *)
intros AT AEQ V F1 F2 x y m H H0.
