...
intros.
induction l; destruct a; simpl; auto; f_equal; auto.
...
induction n; intros; simpl in *; destruct a, b; auto.
destruct a; simpl; auto.
destruct n; simpl.
destruct a; simpl; auto.
autorewrite with lists; auto.
...
induction a; intros; destruct b; simpl in *; try lia; auto.
Qed.
...
destruct l.
lia.
simpl; f_equal; apply IHl; discriminate.
Qed.
...
- inversion H; subst; intros Hcontra; apply H2; apply in_skipn_in in Hcontra; assumption.
- apply IHl; [> inversion H; assumption | assumption].
Qed.
...
rewrite Forall_forall.
intros x Hx.
destruct (in_selN_exists l x def Hx) as [i [Hlt Hi]].
rewrite <- Hi.
apply H.
assumption.
Qed.
...
induction la; intros; destruct lb; simpl in *; try lia; constructor; inversion H0; subst; eauto.
Qed.
...
induction la; intros; destruct lb; simpl in *; try congruence; constructor; auto.
Qed.
...
intros A B a b P n ad bd H Hlen.
revert n Hlen.
induction H; intros.
- inversion Hlen.
- destruct n; simpl.
  + exact H.
  + apply IHForall2. simpl in Hlen. lia.
Qed.
...
(* generated proof tokens: 154 *)

Proof.
  intros A B a b P da db Hlen Hall.
  rewrite <- (map_fst_combine a b Hlen).
  rewrite <- (map_snd_combine a b Hlen).
  generalize dependent b. induction a; intros.
  - simpl in *. destruct b; try discriminate. constructor.
  - simpl in Hlen. destruct b; try discriminate.
    simpl in *.
    constructor.
    + specialize (Hall 0 ltac:(simpl; lia)).
      simpl in Hall. assumption.
    + apply IHa.
      * simpl in Hlen. lia.
      * intros. specialize (Hall (S i) ltac:(simpl; lia)). simpl in Hall. assumption.
Qed.
...
intros T l n k H.
induction l as [| l_hd l_tl IHl_tl] in n, H |- *; simpl in *.
reflexivity.
destruct n; simpl.
(* Reached max number of model calls *)
Admitted.
...
intros T l i i' v H.
induction l; simpl.
destruct i; destruct i'; simpl in *; try lia.
reflexivity.
reflexivity.
destruct i'.
destruct i.
...
split; apply NoDup_rev_1 || apply NoDup_rev_2.
Qed.
...
intros A B a b Hlen.
revert b Hlen; induction a; intros.
- simpl; reflexivity.
-
destruct b; simpl in *; [discriminate | ].
rewrite IHa with (b := b0) by lia.
...
intros T l1 l2 a HNotInA HIncl x HInL1.
(* No more tactics to try *)
Admitted.
...
(* generated proof tokens: 67 *)

intros T l1 l2 a H H0.
unfold incl in *.
intros x H1.
specialize (H x H1).
simpl in H.
destruct H as [H | H].
- exfalso; apply H0; subst; assumption.
- assumption.
Qed.
