...
unfold treeseq_in_ds; split.
split.
split.
unfold tree_rep, tsupd, dsupd; simpl; subst; eauto.
unfold tsupd, treeseq_one_upd in H4; simpl in H4.
unfold tsupd, dsupd, treeseq_one_upd, d_map in H4; simpl in H4.
unfold treeseq_one_upd; simpl in *.
destruct ts as [ts_hd ts_tl]; simpl in *; simpl dsupd in H4; simpl tsupd in H4.
unfold tree_rep in H4; simpl in H4; destruct (find_subtree pathname (TStree ts_hd)) eqn:Hfind; simpl in *; eauto.
destruct d.
eexists; eexists; simpl.
replace (update_subtree pathname
           (TreeFile n
              {| DFData := (DFData d) ⟦ off := v ⟧; DFAttr := DFAttr d |})
           (TStree ts_hd)) with
        (TStree
           {| TStree :=
                update_subtree pathname
                  (TreeFile n
                     {| DFData := (DFData d) ⟦ off := v ⟧; DFAttr := DFAttr d |})
                  (TStree ts_hd);
              TSilist := TSilist ts_hd;
              TSfree := TSfree ts_hd |}).
subst; simpl; auto.
replace (update_subtree pathname
      (TreeFile n
         {|
           DFData := (DFData d) ⟦ off := v ⟧;
           DFAttr := DFAttr d
         |}) (TStree ts_hd)) with
      (TStree
         {|
...
Proof.
  ...
  destruct t.
  simpl.
  destruct (find_subtree pathname TStree0) eqn:Hfind.
