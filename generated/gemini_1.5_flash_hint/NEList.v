...
unfold selR; intros.
destruct (lt_dec n (length l)); auto; try lia.
Qed.
...
unfold popn; intros.
rewrite nthd_oob by auto; rewrite cuttail_oob by auto; reflexivity.
Qed.
...
unfold pushd, d_in; intros; simpl; destruct ds; simpl in *; intuition.
Qed.
...
unfold latest, nthd; intros; destruct l; simpl.
rewrite nth_selN_eq; auto.
(* No more tactics to try *)
Admitted.
...
unfold popn; simpl; intros; rewrite cuttail_length; auto.
Qed.
...
eauto.
reflexivity.
unfold latest, popn; simpl.
destruct n; simpl; try lia.
reflexivity.
unfold nthd, cuttail, latest; simpl.
simpl; destruct n; simpl; try lia; auto.
destruct n; simpl.
destruct l; simpl; try lia; auto.
case_eq (length l - S n); intros; simpl; try lia; auto.
Qed.
...
split; induction 1; intros; try solve [constructor]; try solve [inversion H; constructor]; eauto.
constructor.
constructor.
constructor.
(* No more tactics to try *)
...
apply nelist_subset_oldest_latest'.
apply H.
Qed.
...
unfold nthd, pushd; simpl; intros.
rewrite H.
(* No more tactics to try *)
Admitted.
...
simpl.
destruct n; simpl in *; try lia; auto.
(* No more tactics to try *)
Admitted.
...
(* generated proof tokens: 59 *)
unfold d_in; intros.
destruct H.
exists (fst ds); split; [left|]; auto.
eapply in_map_iff in H; destruct H as [d' [? H']].
...
destruct l; simpl; intros; intuition.
destruct H0; auto.
