...
destruct (AEQ a a0);eauto.
right; eauto.
destruct (AEQ a a0);eauto.
destruct (AEQ a a0).
destruct (AEQ a a0);eauto.
destruct (AEQ a a0); [| congruence].
destruct (AEQ a a0);[ | congruence].
destruct (AEQ a a0);[| congruence].
destruct (AEQ a a0); subst; auto; try congruence.
exists v, l, l'; rewrite upd_eq; auto; try congruence.
...
intros.
unfold possible_sync; intros.
destruct (m a) eqn:Ha; try solve [apply mem_union_sel_none; auto].
destruct (m' a) eqn:Ha'; try solve [apply mem_union_sel_none; auto].
destruct (H a); auto.
destruct (m a) eqn:Hm; try solve [left; simpl; destruct (m' a) eqn:Hm'; simpl; auto].
right; eexists; eexists; eexists; split; [ | split ]; auto.
destruct (m a) eqn:?; try solve [left; simpl; destruct (m' a) eqn:?; simpl; auto].
(* Reached max number of model calls *)
Admitted.
...
induction p; intros; cbn; split; try (constructor; auto); try congruence; intros; inv_opt; subst; try congruence.
...
(* generated proof tokens: 137 *)
intros.
unfold possible_sync in H.
specialize (H a).
...
destruct H1; destruct H2; destruct H8; destruct H5; try congruence.
Qed.
...
exists (fun a : addr => match d a with
                         | None => None
                         | Some (v, vs) => Some (diskval v vs, nil)
                         end).
unfold discard_buffers; intros; destruct (d a); auto; simpl; auto.
...
destruct out; destruct out'; simpl; intuition; try congruence; repeat deex; eauto.
Qed.
