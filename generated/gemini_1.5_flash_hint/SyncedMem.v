...
split; intros.
unfold sm_ds_valid, pushd; simpl; intuition.
...
Proof.
(* original proof tokens: 55 *)
unfold sm_sync_invariant, sm_sync_all; intros.
unfold_sep_star.
exists sm; exists (fun a : addr => match sm a with
                                     | Some _ => Some true
                                     | None => None
                                     end).
split; auto.
apply functional_extensionality; intros; simpl.
destruct (sm x); simpl; auto.
unfold mem_union; simpl in *.
destruct (sm x); simpl; auto.
destruct (sm x); simpl; auto.
unfold mem_union; simpl in *.
destruct (sm x); simpl; auto.
destruct sm; simpl in *; try congruence.
(* No more tactics to try *)
Admitted.
...
unfold sm_sync_invariant; intros.
unfold sm_sync_all; unfold exis; intros; destruct H; eexists; auto.
eapply sm_sync_all_ptsto; eauto.
Qed.
...
unfold sm_sync_invariant, sm_sync_all, emp; intros; auto.
rewrite H; auto.
Qed.
...
eapply sm_sync_invariant_emp; eauto.
(* No more tactics to try *)
Admitted.
...
split; intros; unfold sm_sync_invariant; intros.
apply H.
apply H0.
...
destruct (m1 x); destruct (m2 x); simpl in *; try congruence; auto.
inversion H2; subst; clear H2.
unfold sm_synced.
destruct m1; destruct m2; simpl in *; try congruence.
unfold sm_synced.
unfold mem_union.
destruct (m2 x); simpl; auto.
unfold sm_sync_all in H1.
destruct m1, m2; simpl in *; try congruence.
unfold sm_synced.
destruct m2; simpl in *; try congruence.
...
unfold_sep_star; intuition repeat deex; repeat eexists; eauto.
