...
unfold listpred', sep_star_fold, fold_right; intros.
(* No more tactics to try *)
Admitted.
...
unfold listpred, piff; split; intros; unfold partition in *; simpl in *;
  try (eapply pimpl_trans; [apply listpred_app|]);
  try (eapply pimpl_trans2; [apply listpred_app|]);
  try rewrite ListUtils.partition_eq_filter in *;
  try cancel;
  intros; subst; repeat deex; eauto.
(* No more tactics to try *)
Admitted.
...
split; intros; unfold listmatch; simpl;
  repeat (apply pimpl_sep_star; [apply pimpl_refl|]);
  try (apply listpred_app; auto);
  try (apply listpred_app; auto);
  cancel.
Qed.
...
split; intros; unfold listmatch; simpl; rewrite listpred_listpred'; simpl; repeat deex; cancel.
(* No more tactics to try *)
Admitted.
...
Proof.
(* original proof tokens: 29 *)
unfold listmatch; simpl; intros.
split; intros; unfold listmatch; simpl; autorewrite with lists; try lia; cancel.
(* No more tactics to try *)
Admitted.
...
unfold listpred, piff; split; intros;
...
split; auto.
apply xform_listmatch_idem_l; intros; apply H.
apply xform_listmatch_idem_r; intros; apply H.
Qed.
...
split.
unfold sync_xform, listpred, pimpl; intros;
