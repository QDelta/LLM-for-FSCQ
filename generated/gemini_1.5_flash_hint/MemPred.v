...
intros.
unfold avs2mem; simpl in *; induction avs; intros.
inversion H0; subst.
destruct (HighAEQ (fst a0) a); try solve [eauto].
(* No more tactics to try *)
Admitted.
...
unfold notindomain, avs2mem; simpl; intros.
induction l; simpl; auto.
destruct a0; simpl; intros.
(* No more tactics to try *)
Admitted.
...
induction avs; simpl; intros; auto.
destruct a; simpl; destruct (HighAEQ h a0); try solve [ apply NoDup_cons; auto; eauto using avs_except_nodup ].
(* No more tactics to try *)
Admitted.
...
unfold avs2mem, avs_except; simpl; intros.
destruct avs; simpl; auto.
destruct p; simpl.
destruct (HighAEQ h a); auto.
destruct (HighAEQ h a); auto.
inversion e.
auto.
destruct (HighAEQ h a); auto.
destruct (HighAEQ h a); auto.
destruct (HighAEQ h a); try congruence.
intros.
destruct (HighAEQ h a); subst; auto.
inversion H.
...
induction avs; simpl; intros; auto.
destruct a0; simpl.
destruct (HighAEQ h a); simpl; auto; try congruence.
rewrite IHavs.
(* No more tactics to try *)
Admitted.
...
unfold mem_except, avs2mem; simpl; intros.
apply functional_extensionality; intros.
destruct (HighAEQ x a); auto.
(* No more tactics to try *)
Admitted.
...
unfold mem_pred; unfold mem_pred_one; intros.
eapply pimpl_exists_l; [ | eapply pimpl_exists_r ]; intros.
(* No more tactics to try *)
Admitted.
...
unfold mem_pred; split; intros; xform.
eapply pimpl_exists_r; eexists; eapply pimpl_sep_star; [apply pimpl_refl|]; eauto.
apply xform_listpred.
...
intros; unfold mem_match; intros.
(* No more tactics to try *)
Admitted.
...
(* original proof tokens: 52 *)
intros; unfold mem_match; intros.
destruct (AEQ a0 a); auto; try congruence.
rewrite upd_ne; auto.
intros.
...
destruct (addr_eq_dec a0 a); subst; auto.
right;eauto.
(* No more tactics to try *)
Admitted.
